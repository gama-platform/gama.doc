...
A Skill is basically a **singleton** and **stateless** Java class that:
  ...
  * begins by the annotation [@skill](wikionly#DevelopingIndexAnnotations#skill){.internal-link-anchor}: `@skill(name = "name_of_the_skill_in_gaml")`.
...
To add new attributes to the species that declares this skill, developers have to define them before the class using the [@vars](wikionly#DevelopingIndexAnnotations#vars){.internal-link-anchor} and `@var` annotations.
The `@vars` annotation contains a set of `@var` elements.

In a [@var](wikionly#DevelopingIndexAnnotations#var){.internal-link-anchor} element, one has to define the name, the type and the default value of the attribute. For example in MovingSkill:
```
@vars({
...
### Access to parameters in actions ## {#access-to-parameters-in-actions}
