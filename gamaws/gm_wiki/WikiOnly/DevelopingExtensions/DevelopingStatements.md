# Developing Statements ## {#developing-statements}
...
A new statement must be a Java class that:
  ...
  * begins by the 2 following mandatory annotations:
    * [@symbol](wikionly#DevelopingIndexAnnotations#symbol){.internal-link-anchor}: `@symbol(name = "name_of_the_statement_gaml", kind = "kind_of_statement", with_sequence = true/false)`,
    * [@inside](wikionly#DevelopingIndexAnnotations#inside){.internal-link-anchor}: `@symbol(kinds = {"kind_of_statement_1","kind_of_statement_2","..."}`

In addition the 4 following optional annotations can be added:
  * [@facets](wikionly#DevelopingIndexAnnotations#facets){.internal-link-anchor}: to describe the set of [@facet](wikionly#DevelopingIndexAnnotations#facet){.internal-link-anchor} annotations,
  * [@doc](wikionly#DevelopingIndexAnnotations#doc){.internal-link-anchor}: to document the statement.
  * [@serializer](wikionly#DevelopingIndexAnnotations#serializer){.internal-link-anchor}: in addition, statements can benefit from a custom serializer, by declaring `@serializer(CustomSerializer.class)`, with a class extending `SymbolSerializer`.
  * [@validator](wikionly#DevelopingIndexAnnotations#validator){.internal-link-anchor}: in addition, statements can benefit from a custom validation during the validation process, by declaring `@validator(CustomValidator.class)` with a class implementing `IDescriptionValidator` as value. This class will receive the `IDescription` of the statement and be able to execute further validations on the type of expressions, etc. or even to change the `IDescription`  (by adding new information, changing the value of facets, etc.).
...
### The `action` statement ## {#the-action-statement}
...
### Define a SINGLE\_STATEMENT statement ## {#define-a-single-statement-statement}
...
### Define a statement with sequence ## {#define-a-statement-with-sequence}
...
### Additional methods that can implemented ## {#additional-methods-that-can-implemented}

The following methods have a default implementation, but can be overridden if necessary:
  * the **`String getTrace(final IScope scope)` method** is called to trace the execution of statements using [trace statement](references#Statements#trace){.internal-link-anchor}.
```
