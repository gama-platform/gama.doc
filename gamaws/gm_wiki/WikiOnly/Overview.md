...
The documentation of GAMA is available online on the wiki of the project. It is organized around a few central activities ([installing GAMA](references#Installation), [writing models](wikionly#WritingModels), [running experiments](references#LaunchingExperiments), [developing new extensions to the platform](wikionly#DevelopingExtensions)) and provides complete references on both the [GAML language](wikionly#GamlLanguage), the [platform](Platform) itself, and the scientific aspects of our work (with a complete [bibliography](References)). Several [tutorials](Tutorials) are also provided in the documentation in order to minimize the learning curve, allowing users to build, step by step, the models corresponding to these tutorials, which are of course shipped with the platform.

The documentation can be accessed from the side bar of this page. A good starting point for new users is [the installation page](references#Installation).
...
GAMA can be [downloaded](wikionly#Download) as a regular application or [built from source](https://github.com/gama-platform/gama), which is necessary if you want to contribute to the platform.
...
It is, in any case, recommended to follow the instructions on [this page](wikionly#InstallingGitVersion) in order to build GAMA from source.
...
Finally, if you think you have found a bug in GAMA, or if you absolutely need a feature that does not exist yet, it is much more efficient and time-saving for everyone (including current and future users) to create a new issue report. Please refer to [these instructions](references#Troubleshooting#Submitting_an_Issue){.internal-link-anchor} to do so.