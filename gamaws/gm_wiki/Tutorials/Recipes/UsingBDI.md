...
## Acteur Projet ## {#acteur-projet}
A website (still in construction) of the ACTEUR project can be found here http://acteur-anr.fr/


## An introduction to cognitive agent ## {#an-introduction-to-cognitive-agent}
...
## Basic Example: A fire rescue model using cognitive agent ## {#basic-example-a-fire-rescue-model-using-cognitive-agent}
...
First let's create a BDI agent using the key control **simple\_bdi** (A description of all existing control architectures is available [here](references#BuiltInArchitectures).)

### Species Helicopter creation ## {#species-helicopter-creation}
...
##### Aspect of the helicopter ## {#aspect-of-the-helicopter}
```
aspect base {
...
##### FireArea Species ## {#firearea-species}
```
species fireArea{
        float size <-1.0;	
...
##### WaterArea Species ## {#waterarea-species}
```
species waterArea{
	float size <-10.0;
