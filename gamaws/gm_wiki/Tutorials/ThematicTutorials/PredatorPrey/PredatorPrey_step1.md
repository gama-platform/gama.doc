...
#### Prey species code ## {#prey-species-code}
...
It allows to initialize simulations (init block): the world is always created and initialized first when a simulation is launched (before any other agents). The geometry (shape) of the world agent is by default a square with 100m for side size, but can be redefined if necessary (see the [Road traffic tutorial](tutorials#RoadTrafficModel)).
...
Note that it is possible to define a [opengl display](tutorials#Defining3DDisplays) (for 3D display) by using the facet **type: opengl**.
