# 1. Simple SI Model ## {#1-simple-si-model}
...
![images/Incremental_model1.jpg](gm_wiki/resources/images/tutorials/Incremental_model1.jpg){.img-responsive}
...
Concerning the internal state, a [attribute](tutorials#RegularSpecies#declaration){.internal-link-anchor} is defined as follows: type of the attribute (int (integer), float (floating point number), string, bool (boolean, true or false), point (coordinates), list, pair, map, file, matrix, espèce d’agents, rgb (color), graph, path...) + name
...
Concerning the display of an agent, [aspects](tutorials#RegularSpecies#the-aspect-statement){.internal-link-anchor} have to be defined. An aspect represents a possible way to display the agents of a species : aspect aspect\_name {…}
